            ...
            <div class="右侧正文">
                <div class="订单页面">
                    ...
                    <div class="订单列表内容">
                        ...
                        border
                        height="700">
                            <el-table-column label="订单号" prop="orderId" header-align=center align="center" width="80px">
                            ...
                            <el-table-column label="总金额" header-align=center align="center" width="80px">
                                <template slot-scope="scope">
                                    <strong style="color:red">{{scope.row.orderPaymentMoney.toFixed(2)}}</strong>
                                </template>
                            </el-table-column>
                            <el-table-column label="买家名称" header-align=center align="center" width="80px">
                                <template slot-scope="scope">
                                    <strong style="color:black">{{scope.row.username}}</strong>
                                </template>
                            </el-table-column>  
                            <el-table-column label="订单状态" header-align=center align="center" width="80px">
                                <template slot-scope="scope">
                            ...
                            <el-table-column label="操作" header-align=center align="center" prop="quantity">
                                <template slot-scope="scope">
                                        ...
                                        <div class="订单按钮">
                                            <el-button v-if="scope.row.orderStatus==2" type="success" @click="payorder(scope.$index)">
                                                订单发货
                                            </el-button>
                                        </div>
                                        <div class="订单按钮">
...
export default {
    ...
    methods: {
        payorder(index){
            localStorage.waitsendorderid=this.订单列表[index].orderId;
            localStorage.waitsendordername=this.订单列表[index].username;
        ...
        updateorder(){
            ...
            JSON.stringify(subform)
             axios.defaults.headers.common['Authorization']=JSON.parse(localStorage.currentuser).token;
            axios.post("http://localhost:9090/order/all/information/order/seller",subform).then(resp=>{
